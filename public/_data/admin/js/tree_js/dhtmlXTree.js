...
this._itim_dg=true;this.dlmtr=",";
this.dropLower=false;this.enableIEImageFix();
this.xmlstate=0;this.mytype="tree";
this.smcheck=true;this.width=width;
this.height=height;this.rootId=rootId;
this.childCalc=null;this.def_img_x="18px";
this.def_img_y="18px";this.def_line_img_x="18px";
...
this.lWin=window;this.cMenu=0;
this.mlitems=0;this.dadmode=0;
...
td2.colSpan=3;td2.appendChild(htmlObject);
...
if(image3)Nodes[Count].images[2]=image3;parentObject.childsCount++;
...
if (this.XMLsource)
if ((childs)&&(childs!=0)) Nodes[Count].XMLload=0;
...
break;
case "CALL": this.selectItem(itemId,true);
break;
...
if (style)
if (newNode.span.style.cssText)newNode.span.style.cssText+=(";"+style);
else
...
if (zcall!="")nodeAskingCall=zcall;npl++;}
else
 dhtmlObject.checkUserData(node.childNodes[i],parentId);};
if (!level){
if (dhtmlObject.waitUpdateXML){dhtmlObject.waitUpdateXML=false;
for (var i=temp.childsCount-1;i>=0;i--)
if (temp.childNodes[i]._dmark)dhtmlObject.deleteItem(temp.childNodes[i].id);};
...
if (temp.checkstate===0)dhtmlObject._setSubChecked(0,temp);
else if (temp.checkstate===1)dhtmlObject._setSubChecked(1,temp);};
if(navigator.appVersion.indexOf("MSIE")!=-1 && navigator.appVersion.indexOf("5.5")!=-1){
window.setTimeout(function(){
dhtmlObject._redrawFrom(dhtmlObject,null,start)},10);}
else{
...
if ((!tempx.unParsed)&&((tempx.XMLload)||(!this.XMLsource)))
 tempx._acc=acc;dhtmlObject._correctLine(tempx);
...
var tempNum=2;if (!itemObject.treeNod.treeLinesOn)this._setSrc(imsrc,this.imPath+workArray[3]);
else {if (itemObject.parentObject)tempNum=this._getCountStatus(itemObject.id,itemObject.parentObject);
...
if (itemObject.childsCount<=1){if (itemObject.id==this.rootId)return 4;
else return 0;};
...
if ((this.XMLsource)&&(!itemObject.XMLload)) {if (mode==1)return;
...
else{that._oie_onXLE=that.onXLE;that.onXLE=that._epnFHe;};};
...
dhtmlXTreeObject.prototype.onRowClickDown=function(e){e=e||window.event;
var that=this.parentObject.treeNod;that._selectItem(this.parentObject,e);};
...
var z=this._globalIdStorageFind(itemId);if (!z)return null;
...
e=e||window.event;var obj=this.parentObject;
...
var table=document.createElement('table');table.cellSpacing=0;table.cellPadding=0;table.border=0;
...
if ((!this.cBROf)&&(inp.tagName=="IMG"))
 inp.align="absmiddle";inp.onclick=this.onCheckBoxClick;
inp.treeNod=this;
...
img.border="0";
if (this._aimgs){img.parentObject=itemObject;
...
tr.appendChild(td1);tr.appendChild(td11);
tr.appendChild(td12);
tr.appendChild(td2);
...
dhtmlXTreeObject.prototype.changeItemId=function(itemId,newItemId)
 {var temp=this._globalIdStorageFind(itemId);
if (!temp)return 0;
temp.id=newItemId;temp.span.contextMenuId=newItemId;
...
var _otiid=itemObject.id;itemObject=this._recreateBranch(itemObject,targetObject,beforeNode);
...
if ((document.all)&&(navigator.appVersion.search(/MSIE\ 5\.0/gi)!=-1))
 {window.setTimeout(function() {
zir.parentNode.removeChild(zir);}, 250 );}
else 
...
itemObject.span.innerHTML=itemObject.label;itemObject.i_sel=false;
...
if (!this.xmlstate)this._epnFH(item.id,this._getOpenState(item));
else{this._oie_onXLE=this.onXLE;
...
var a=0;for (var b=0;b<itemNode.childsCount;b++)
...
if (this.autoScroll){
if (node){a1=getAbsoluteTop(node);
...
else var col=z._dragged;var trg=targetHtmlObject.parentObject;
...
var tree=that.treeNod;tree.callEvent("onMouseOut",[that.id]);
...
if (tree._l_onMSI!=that.id)tree.callEvent("onMouseIn",[that.id]);tree._l_onMSI=that.id;
if (!tree.ehlt)
return;that.span.className=that.span.className.replace("_lor","");
