 ...
 * == END LICENSE ==
 */
namespace Soulex\File;

use finfo;
use Soulex\File\HttpUpload;
use Soulex\File\HttpUpload\Filter;
use Soulex\File\HttpUpload\Exception;
use Soulex\File\HttpUpload\FilePath;
...
class HttpUpload
{
    ...
    /**
     * @var \Soulex\File\HttpUpload\FilePath
     */
     ...
     *
     * @return \Soulex\File\HttpUpload object
     *
    ...
    public function addFilter($filterName)
    {
        ...
        if (!file_exists($filterFile)) {
            throw new Exception('Filter ' . $filterFile . ' not found');
        }

        $className = __NAMESPACE__ . "\\HttpUpload\\Filter\\" . $filterName . "\\Filter";

        if (!class_exists($className)) {
            throw new Exception('Filter ' . $className . ' doesn\'t exist');
        }

        $filterObject = new $className;

        if (!($filterObject instanceof FilterInterface)) {
            throw new Exception('Filter ' . $className . ' does not implement \Soulex\File\HttpUpload\Filter\FilterInterface');
        }
    ...
    public function applyFilter($filterArray, $filter, $params = null)
    {
        ...
        if (!isset($this->loaded_filters[$filterName])) {
            throw new Exception('Apply filter error: filter ' . $filterName . ' is not loaded');
        }
    ...
    public function filter($filterName)
    {
        ...
        if (!isset($this->loaded_filters[$filterName])) {
            throw new Exception('Can not get filter ' . $filterName);
        }
    ...
    {
        // включаем запрет файла по маске
    ...
    /**
     * @return \Soulex\File\HttpUpload\FilePath
     */
    protected function getPathManager()
    {
        if (null === $this->pathManager) {
            $this->pathManager = new FilePath();
        }
    ...
    private function init()
    {
        ...
        if (!isset($_FILES[$this->field_name]) || !is_array($_FILES[$this->field_name])) {
            throw new Exception('Error while initializing: No data received');
        }
        ...
        if (!is_dir($dir_to_upload)) {
            ...
            if ($this->auto_create_dir === true) {
                ...
                if (!@mkdir($dir_to_upload, 0777, true)) {
                    throw new Exception('Directory creation failed: ' . $dir_to_upload . '!');
                }
                umask($oldumask);
            } else {
                throw new Exception('Directory ' . $dir_to_upload . ' doesn\'t exist');
            }
        }
    ...
    private function check_for_random_name()
    {
        if (empty($this->filename)) {
            throw new Exception('No filename specified for randomizing');
        }
    ...
    private function check_mime_allowed()
    {
        ...
        if (empty($this->file_mime)) {
            throw new Exception('File MIME type not specified');
        }
        ...
        if ($allow === false) {
            throw new Exception('File\'s MIME type is not allowed');
        }
     ...
     *
     * @param string $mime разрешенные mime-типы (является элементом массива $this->allowed)
     * @param string $op массив содержащий тип проверки и результат проверки
    ...
    private function is_mime_allowed($mime, $op)
    {
        ...
        if (($v1 == '*' && $v2 == '*') || ($v1 == $m1 && ($v2 == $m2 || $v2 == '*'))) {
            ...
            elseif ($op == self::CHECK_MIME_FORBIDDEN)
                ...
                throw new Exception('Unknown check mime operation ' . $op);
        }
    ...
    private function check_forbidden_file_name_mask()
    {
        ...
        if ($this->forbidden_file_name_mask_enabled === true) {
            ...
            if ($allow === false) {
                throw new Exception('File is not allowed to upload');
            }
        }
        return $this;
    }
     ...
     *
     * @param string $mask маска файла, заданная в виде регулярного выражения
     * @return TRUE /FALSE
     * @internal param object $index индекс массива
     * @internal param object $allow результат проверки
     */
    ...
    private function check_upload_errors()
    {
        ...
        if (array_key_exists($this->file_error, $this->upload_err_messages)) {
            ...
            if ($_err_array['err_msg'] != 'UPLOAD_ERR_OK') {
                throw new Exception($_err_array['err_msg'] . ': ' . $_err_array['err_desc']);
            }
        } else {
            throw new Exception('Upload failed: unknown error');
        }
    ...
    private function check_max_file_size()
    {
        if (!empty($this->max_file_size)) {
            if ((int)$this->max_file_size == 0) {
                throw new Exception('Wrong number specified for max_file_size');
            }
            ...
            if ($this->file_size >= $size) {
                throw new Exception('File size can not be more than ' . $this->max_file_size);
            }
        }
